 ...
 *
 */
public class vc0101Popover extends ScsbPopover {
	...
	public vc0101Popover(String title ,FilterTable filterable) {
		this.title=title;
	...
	public vc0101Popover(Filterable filterable) {
		for(java.util.Iterator<?> iter=filterable.getItemIds().iterator();iter.hasNext();){
			Object itemId =iter.next();
			Item item =filterable.getItem(itemId);
			item.getItemProperty("resp_no");
	...
	public vc0101Popover(JsonContainer jsonData){
	...
	private void init(){
		...
		this.setTitle("<font color='red'><b>"+title+i18nProps.getProperty("title","AO績效")+"</b>&nbsp;</font>");
		
		convertChart conChart =new convertChart();
		vchart.removeAllComponents();
		//柱狀圖====================================================
		...
		vchart.addComponent(chart1.getHighChart());
		...
		vchart.addComponent(chart2.getHighChart());
		this.addContentComponent(vchart);
	}
}