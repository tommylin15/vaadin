...
public class OneCaseFileCode {
	...
	public OneCaseFileCode(String serviceName ,String servicePackage 
		...
		if (container.size() > 0){
			...
			sCode+="import com.scsb.db.bean.InBox;"+BR;
			sCode+="import com.scsb.domain.CmCaseMemService;"+BR;		
	...
	public String getCreate(){
		...
		sCode+="	    //案件資料"+BR;
		sCode+="	    CaseLayout vCaseData =new CaseLayout(session);"+BR;
		sCode+="	    bodyLayout.addComponent(vCaseData);"+BR;
	...
	public String getDetailInsertAction(){	
		...
		sCode+="	 	newBean.setCaseSn(caseSn);"+BR;
	...
	public String getQueryAction(){	
		...
		sCode+="	 	//從db取資料"+BR;
		sCode+="		DetailData =new BeanContainer<String, CmCaseMem>(CmCaseMem.class);"+BR;
		sCode+="		DetailData.setBeanIdProperty(\"beanKey\");"+BR;
		sCode+="		DetailData.removeAllItems();"+BR;
		sCode+="		//取得 case_cust by case_sn"+BR;
		sCode+="		InBox inbox =(InBox) session.getAttribute(\"_InBox\");"+BR;
		sCode+="		if (inbox !=null){"+BR;
		sCode+="			caseSn =inbox.getCaseSn();"+BR;
		sCode+="			DetailData =DataSrv.getCmCaseMem_caseSn(caseSn);"+BR;
		sCode+="			if(DetailData.size()==0){"+BR;
		sCode+="				// 取不到case main時	"+BR;
		sCode+="				Label noCaseMain =new Label(\"查無此案件編號:\"+inbox.getCaseSn()+\"\");"+BR;
		sCode+="				bodyLayout.removeAllComponents();"+BR;
		sCode+="				bodyLayout.addComponent(noCaseMain);"+BR;
		sCode+="				bodyLayout.setComponentAlignment(noCaseMain, Alignment.TOP_CENTER);"+BR;
		sCode+="				return;"+BR;
		sCode+="			}else{"+BR;
