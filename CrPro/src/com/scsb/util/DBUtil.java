...
public class DBUtil {
        ...
        try {
			...
			for(Iterator<Hashtable<String,Object>> iterator = hashSet.iterator(); iterator.hasNext(); ){
				...
				if (hashData.get("DATA_TYPE").equals("int")){
					...
					double  dValue =Double.parseDouble(hashData.get("VALUE").toString());
					dbAction.prepareStatement.setDouble(iIndex, dValue);					
				}else if (hashData.get("DATA_TYPE").equals("object")){
					dbAction.prepareStatement.setObject(iIndex, hashData.get("VALUE"));					
				}else{					
					dbAction.prepareStatement.setString(iIndex, hashData.get("VALUE").toString());
				}
			};			
	       // Initialize statement and execute the query
			...
			IndexedContainer      container  =getContainerFmt(dbAction.resultSet.getMetaData());
    		int iRows=0;
    		int iTotRows=0;			
			while(dbAction.resultSet.next()){
				iRows++;
				iTotRows++;
				Item item =container.addItem(iRows);
				for(int i = 1; i <= dbAction.resultSet.getMetaData().getColumnCount(); i++){
					String fieldName=dbAction.resultSet.getMetaData().getColumnName(i).toUpperCase();
					String fieldType =dbAction.resultSet.getMetaData().getColumnTypeName(i).toUpperCase();
					if (fieldType.equals("DATE"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getDate(i));
					else if (fieldType.equals("NUMBER"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getDouble(i));
					else if (fieldType.equals("INT"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getInt(i));
					else if (fieldType.equals("INTEGER"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getInt(i));
					else if (fieldType.equals("FLOAT"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getFloat(i));
					else if (fieldType.equals("TIMESTAMP"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getTimestamp(i));
					else if (fieldType.equals("DOUBLE"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getDouble(i));
					else if (fieldType.equals("LONG")){
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getString(i));
					}else if (fieldType.equals("LONGVARCHAR"))
						item.getItemProperty(fieldName).setValue(dbAction.resultSet.getString(i));
					else {
						String sObj=dbAction.resultSet.getString(i);
						if (sObj != null){
	 ...
	 * @throws SQLException
	 */
	private static IndexedContainer getContainerFmt(ResultSetMetaData  rsMeta) throws SQLException{
    	int numCols=0;        	
